<div x-show="cableFailureEnabled" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
    ...
    <fieldset :disabled="!cableFailureEnabled">
    <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <select id="cf_type_storing" name="cable_failure[type_storing]" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md">
                <option value="">-</option>
                <option value="Kabelbreuk" @selected(old('cable_failure.type_storing')==='Kabelbreuk')>Kabelbreuk</option>
                <option value="Slechte verbinding" @selected(old('cable_failure.type_storing')==='Slechte verbinding')>Slechte verbinding</option>
                <option value="Kortsluiting" @selected(old('cable_failure.type_storing')==='Kortsluiting')>Kortsluiting</option>
                <option value="Overig" @selected(old('cable_failure.type_storing')==='Overig')>Overig</option>
            </select>
        ...
        <div>
            ...
            <x-text-input id="cf_locatie_storing" name="cable_failure[locatie_storing]" type="text" class="block mt-1 w-full" :value="old('cable_failure.locatie_storing')" />
        ...
        <div>
            ...
            <select id="cf_kabel_met_aftakking" name="cable_failure[kabel_met_aftakking]" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md">
                <option value="0" @selected(old('cable_failure.kabel_met_aftakking')==='0')>Nee</option>
                <option value="1" @selected(old('cable_failure.kabel_met_aftakking')==='1')>Ja</option>
            </select>
        ...
        <div class="md:col-span-2">
            ...
            <textarea id="cf_bijzonderheden" name="cable_failure[bijzonderheden]" rows="4" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md">{{ old('cable_failure.bijzonderheden') }}</textarea>
            <x-input-error :messages="$errors->get('cable_failure.bijzonderheden')" class="mt-2" />
        </div>

        <div class="md:col-span-2">
            <x-input-label for="cf_advies" value="Advies" />
            <textarea id="cf_advies" name="cable_failure[advies]" rows="4" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md">{{ old('cable_failure.advies') }}</textarea>
